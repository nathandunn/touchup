...
public class PhyloXmlParser implements PhylogenyParser {
    ...
    private Reader getReaderFromZipFile() throws IOException {
        ...
        while ( zip_file_entries.hasMoreElements() ) {
            ...
            if ( !zip_file_entry.isDirectory() && ( zip_file_entry.getSize() > 0 ) ) {
                ...
                reader = new InputStreamReader( is, PhyloConstant.UTF_8 );
                break;
            }
        }
        try {
    ...
    @Override
    public Tree[] parse() throws IOException, PhylogenyParserException {
        reset();
        final PhyloXmlHandler handler = new PhyloXmlHandler();
        final SAXParserFactory factory = SAXParserFactory.newInstance();
        factory.setNamespaceAware( true );
        try {
            if ( !PhyloUtil.isEmpty( getSchemaLocation() ) ) {
        ...
        try {
            ...
            if ( getSource() instanceof File ) {
                if ( !getSource().toString().toLowerCase().endsWith( ".zip" ) ) {
                    ...
                    final InputStreamReader isr = new InputStreamReader( is, PhyloConstant.UTF_8 );
                ...
                is.setEncoding( PhyloConstant.UTF_8 );
                xml_reader.parse( is );
            }
            else if ( getSource() instanceof InputStream ) {
                if ( !isZippedInputstream() ) {
                    final InputStream is = ( InputStream ) getSource();
                    xml_reader.parse( new InputSource( new BufferedReader( new InputStreamReader( is, PhyloConstant.UTF_8 ) ) ) );
                }
                else {
                    final ZipInputStream zip_is = new ZipInputStream( ( InputStream ) getSource() );
                    zip_is.getNextEntry();
                    xml_reader.parse( new InputSource( new BufferedReader( new InputStreamReader( zip_is, PhyloConstant.UTF_8 ) ) ) );
                }
            }
        ...
        final Tree[] ps = new Tree[ handler.getPhylogenies().size() ];
        int i = 0;
        for( final Tree phylogeny : handler.getPhylogenies() ) {
            ps[ i++ ] = phylogeny;
        }
        return ps;
    }
    ...
    public static PhyloXmlParser createPhyloXmlParserXsdValidating() {
        ...
        final URL xsd_url = cl.getResource( PhyloConstant.LOCAL_PHYLOXML_XSD_RESOURCE );
        if ( xsd_url != null ) {
            ...
            throw new RuntimeException( "failed to get URL for phyloXML XSD from jar file from ["
                    + PhyloConstant.LOCAL_PHYLOXML_XSD_RESOURCE + "]" );
        }
        return xml_parser;
    }
    ...
    private class PhyloXmlParserErrorHandler extends DefaultHandler {
        ...
        public void warning( final SAXParseException e ) {
            ...
            if ( _error_messages.length() > 1 ) {
                _error_messages.append( PhyloUtil.LINE_SEPARATOR );
            }
