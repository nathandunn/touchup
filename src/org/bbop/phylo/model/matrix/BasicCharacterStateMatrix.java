...
package org.bbop.phylo.model.matrix;
...
import org.bbop.phylo.io.IllegalFormatUseException;
...
public class BasicCharacterStateMatrix<S> implements CharacterStateMatrix<S> {
    ...
    private void toForester( final Writer writer ) throws IOException {
        ...
        writer.write( PhyloUtil.LINE_SEPARATOR );
        writer.write( PhyloUtil.pad( "", 20, ' ', false ).toString() );
        writer.write( ' ' );
        for( int character = 0; character < getNumberOfCharacters(); ++character ) {
            final String c = getCharacter( character );
            writer.write( c != null ? PhyloUtil.pad( c, longest, ' ', false ).toString() : PhyloUtil
        ...
        for( int identifier = 0; identifier < getNumberOfIdentifiers(); ++identifier ) {
            if ( getIdentifier( identifier ) != null ) {
                writer.write( PhyloUtil.pad( getIdentifier( identifier ), 20, ' ', false ).toString() );
                writer.write( ' ' );
            }
            for( int character = 0; character < getNumberOfCharacters(); ++character ) {
                final S state = getState( identifier, character );
                writer.write( state != null ? PhyloUtil.pad( state.toString(), longest, ' ', false ).toString()
                        : PhyloUtil.pad( "", longest, ' ', false ).toString() );
                if ( character < ( getNumberOfCharacters() - 1 ) ) {
                    writer.write( ' ' );
                }
            }
    ...
    private void toPhylip( final Writer writer ) throws IOException {
        ...
        writer.write( PhyloUtil.LINE_SEPARATOR );
        for( int identifier = 0; identifier < getNumberOfIdentifiers(); ++identifier ) {
            if ( !PhyloUtil.isEmpty( getIdentifier( identifier ) ) ) {
                writer.write( PhyloUtil.pad( getIdentifier( identifier ), pad, ' ', false ).toString() );
                writer.write( ' ' );
                writer.write( ' ' );
            }
            else {
            ...
            for( int character = 0; character < getNumberOfCharacters(); ++character ) {
                ...
                writer.write( state != null ? PhyloUtil.pad( state, pad, ' ', false ).toString() : PhyloUtil
    ...
    public void writeNexusMatrix( final Writer w ) throws IOException {
        ...
        for( int identifier = 0; identifier < getNumberOfIdentifiers(); ++identifier ) {
            if ( getIdentifier( identifier ) != null ) {
                w.write( "  " );
                w.write( PhyloUtil.pad( getIdentifier( identifier ), 20, ' ', false ).toString() );
                w.write( ' ' );
            }
            for( int character = 0; character < getNumberOfCharacters(); ++character ) {
                final S state = getState( identifier, character );
